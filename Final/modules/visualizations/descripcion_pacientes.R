crear_grafico_descripcion_pacientes <- function(input, output, session, data, ranges, click_info) {
  ...
  columnas_mostrar <- c(
    "Paciente", "Edad", "Sexo", "Realiza control regular de la presión?",
    "Minutos semanales de actividad aeróbica", "Dieta Mediterránea",
    "Antecedentes familiares de cardiopatía isquémica precoz (menor de 50 en varones y menor de 55 en mujeres)",
    "Riesgo de caída", "Caidas previas conocidas", "Toma algún medicamento presente en esta lista",
    "Deficit sensorial", "Estado mental", "Deambulación alterada", "Tabaquismo",
    "Consumo diario de alcohol", "Hipertensión arterial", "Max valor P. Sistólica",
    "Hypertensión controlada", "Medicamento Antihipertensivo", "Valvulopatía",
    "Enfermedad coronaria", "Tipo", "Cuantos?", "Tipo de intervencion",
    "Enfermedad aórtica", "Enfermedad carótidea", "Enfermedad vascular periférica",
    "Arritmias", "Arritmia auricular", "Taquicardia ventricular",
    "Taquicardia paroxística supraventicular", "Muerte súbita cardíaca", "Ablación",
    "Insuficiencia cardíaca", "Etiología", "NYHA", "Fracción de Eyección del Ventrículo Izquierdo",
    "Hipertensión pulmonar", "Presencia de crepitantes", "Ingurgitación de vena yugular",
    "Transplante cardíaco", "Resincronización", "Miocardiopatía"
  )

  # Filtrar datos según edad y género
  datos_filtrados <- reactive({
    datos %>%
      filter(Edad >= input$edad[1], Edad <= input$edad[2]) %>%
      filter(Sexo == input$sexo | input$sexo == "Todos")
  })

  crear_combinacion_factores <- reactive({
    req(input$factores)
    datos_filtrados() %>%
      mutate(Grupo_Factores = apply(select(., all_of(input$factores)), 1, paste, collapse = " & "))
  })
